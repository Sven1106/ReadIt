// Modules
const { ipcRenderer } = require('electron')
const items = require('./items')
const menu = require('./menu')

// Navigate selected item with up

$(document).keydown((e) => {
    switch (e.key) {
        case 'ArrowUp':
            items.changeItem('up')
            break;

        case 'ArrowDown':
            items.changeItem('down')

            break;
        default:
            break;
    }
})





// Show add-modal
$('.open-add-modal').click(() => {
    $('#add-modal').addClass('is-active')
    $('#item-input').focus()

})

// Hide add-modal
$('.close-add-modal').click(() => {
    $('#add-modal').removeClass('is-active')
    $('#item-input').removeClass('is-danger')
    $('#item-input').prop('disabled', false).val('')
})

// Handle add-modal submission
$('#add-button').click(() => {
    // Get Url from input
    let newItemURL = $('#item-input').val()
    if (newItemURL) {
        fetch(newItemURL)
            .then(function () {
                // Disable modal ui
                $('#item-input').removeClass('is-danger')
                $('#item-input').prop('disabled', true)
                $('#add-button').addClass('is-loading')
                $('.close-add-modal').addClass('is-disabled')
                // Send URL to main process via IPC
                ipcRenderer.send('new-item', newItemURL)
            })
            .catch(function () {
                $('#item-input').addClass('is-danger')
                $('.modal-content').addClass('shake');
                $('.modal-content').on('webkitAnimationEnd oanimationend msAnimationEnd animationend', function (e) {
                    $('.modal-content').delay(200).removeClass('shake');
                });
            });

    }
})


// Simulate add click on enter
$('#item-input').keyup((e) => {
    if (e.key === 'Enter') {
        $('#add-button').click()
    }
})

// Filter items by title
$('#search').keyup((e) => {

    // Get current #search input value
    let filter = $(e.currentTarget).val()
    console.log(filter)
    $('.read-item').each((i, el) => {
        $(el).text().toLowerCase().includes(filter) ? $(el).show() : $(el).hide()
    })
})

// Add items when app starts
if (items.toreadItems.length) {
    items.toreadItems.forEach(items.addItem);
    $('.read-item:first()').addClass('is-active')
}

// Listen for new item for main
ipcRenderer.on('new-item-success', (e, item) => {
    // Add item to Items Array
    items.toreadItems.push(item)

    // Save items
    items.saveItems()

    // Add item
    items.addItem(item)

    // Close and reset modal
    $('#add-modal').removeClass('is-active')
    $('#item-input').prop('disabled', false).val('')
    $('#add-button').removeClass('is-loading')
    $('.close-add-modal').removeClass('is-disabled')

    // If first item being added, select it
    if (items.toreadItems.length === 1) {
        $('.read-item:first()').addClass('is-active')
    }
})


